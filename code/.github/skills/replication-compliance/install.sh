...
#   SKILL_REPO_REF=<branch> ...          # Optional branch override for testing
#
...
if [ -f "$SCRIPT_DIR/SKILL.md" ]; then
    ...
    echo "Downloading skill from github.com/$REPO ($REPO_REF) ..."
    TMPDIR_PATH=$(mktemp -d)
    git clone --branch "$REPO_REF" --single-branch --filter=blob:none --sparse --quiet \
